...
import { generateZwoXml, CyclingDay } from "@/lib/erg";
...
/**
 * Exporta los entrenamientos de ciclismo de las rutinas activas como archivos
 * .zwo (Zwift) listos para subir a MyWhoosh desde https://workout.mywhoosh.com.
 * Mismo modelo de bloques que el export .fit (iGPSPORT/Garmin); solo cambia el
...
export async function GET(request: Request) {
  try {
    ...
    for (const day of cyclingDays) {
      ...
      zip.file(`${base}.zwo`, generateZwoXml(day));
    }

    zip.file(
      "LEEME.txt",
      `Entrenamientos de ciclismo para MyWhoosh - CoachIA
=============================================
...
Formato .zwo (Zwift) — importable en MyWhoosh:
  1. Entrá a https://workout.mywhoosh.com con tu cuenta.
  2. En "Workouts", tocá el botón de subir (upload) y elegí un archivo .zwo.
  3. El workout queda en tu carpeta de custom workouts dentro de la app.
  4. Reiniciá la app de MyWhoosh para que aparezca el workout nuevo.

La potencia va como fracción de tu FTP (ej. 0.65 = 65%), así que asegurate de
tener tu FTP configurado en MyWhoosh para que los targets salgan en watts reales.
`
    );
    ...
    return new NextResponse(blob, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="coachia-mywhoosh-${new Date().toISOString().slice(0, 10)}.zip"`,
      },
    });
  } catch (err: any) {
    console.error("Export ZWO error:", err);
    return NextResponse.json(
